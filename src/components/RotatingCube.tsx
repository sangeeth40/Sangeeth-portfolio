...
const HALF            = SIZE / 2
const AUTO_SPEED      = 0.40   // deg/frame during idle spin
...
const FACE_BASE: CSSProperties = { position: 'absolute', width: SIZE, height: SIZE, top: 0, left: 0 }

// Each face: its CSS transform within preserve-3d space, border and background colours
const FACES = [
  { transform: `translateZ(${HALF}px)`,                 border: 'rgba(99,102,241,0.60)',  bg: 'rgba(99,102,241,0.05)'  },
  { transform: `rotateY(180deg) translateZ(${HALF}px)`, border: 'rgba(99,102,241,0.48)',  bg: 'rgba(99,102,241,0.04)'  },
  { transform: `rotateY(-90deg) translateZ(${HALF}px)`, border: 'rgba(139,92,246,0.60)', bg: 'rgba(139,92,246,0.05)'  },
  { transform: `rotateY(90deg)  translateZ(${HALF}px)`, border: 'rgba(139,92,246,0.48)', bg: 'rgba(139,92,246,0.04)'  },
  { transform: `rotateX(-90deg) translateZ(${HALF}px)`, border: 'rgba(34,211,238,0.60)',  bg: 'rgba(34,211,238,0.05)'  },
  { transform: `rotateX(90deg)  translateZ(${HALF}px)`,  border: 'rgba(34,211,238,0.48)',  bg: 'rgba(34,211,238,0.04)'  },
]
...
export function RotatingCube() {
    ...
    const tick = () => {
      if (!isDragging.current) {
        if (isAutoSpin.current) {
          ...
          const settled = Math.abs(velX.current) < 0.05 && Math.abs(velY.current) < 0.05
  ...
  return (
    <div className="float-animation flex flex-col items-center gap-3 select-none">
      ...
      <div
        ref={containerRef}
        style={{ width: SIZE, height: SIZE, perspective: `${SIZE * 6}px`, cursor: 'grab', touchAction: 'none', userSelect: 'none' }}
      ...
      <span className="text-[11px] text-slate-600 tracking-widest uppercase pointer-events-none">
        drag to spin
      </span>
    </div>
  )
